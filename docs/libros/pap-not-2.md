---
title: "Teoría de singularidades en topología, geometría y foliaciones II"
authors: [[['Jean-Paul'], ['Brasselet']], [['Felipe'], ['Cano']], [['Dominique'], ['Cerveau']], [['Dung', 'Tráng'], ['Lê']], [['Frank'], ['Loray']], [['Mutsuo'], ['Oka']], [['José'], ['Seade']], [['Mark'], ['Spivakovsky']]]
tags: [Papirhos, Notas, 2999.0]
---
# Teoría de singularidades en topología, geometría y foliaciones II
<div class = "chips"><span class ="chip"></span class ="icon">🏷</span> Notas</span> <span class ="chip"></span class ="icon">📚</span> Papirhos</span> <span class ="chip"></span class ="icon">🗓</span> 2999.0</span> <span class ="chip"></span class ="icon">ℹ️</span> Físico</span></div>

<p align = "left"> <img src = "../../assets/covers/pap-not-2.jpg" width="500" height="600"></p>

## Resumen
Resumen proximamente

## Metadatos
|  |  |
|---|---|
| **Autores** | Jean-Paul Brasselet, Felipe Cano, Dominique Cerveau, Dung, Tráng Lê, Frank Loray, Mutsuo Oka, José Seade, Mark Spivakovsky | 
| **Colección** | Papirhos | 
| **Serie** | Notas | 
| **Año** | 2999.0 | 
| **Editorial** | Instituto de Matemáticas, UNAM | 
| **Edición** | 0.0 | 
| **ISBN (Colección)** | 000 | 
| **ISBN (Texto)** | 000 |

## Descargas
<a class="md-button data-book-id=pap-not-2 download-link" data-book-id="pap-not-2" href = "pap-not-2_mark.pdf" target = "_blank" rel ="noopener" > Abrir PDF </a>
<a class="md-button  data-book-id=pap-not-2 download-link" data-book-id="pap-not-2" href ="pap-not-2_mark.pdf" download> Descargar</a>
<details>
<summary> Ver en línea (vista previa)</summary>
<object data = "pap-not-2_mark.pdf" type="application/pdf" width="100%" height="700" >
<p> Tu navegador no puede mostrar PDF incrustado <a href="pap-not-2_mark.pdf" target="_blank" rel ="noopener"> Abrir PDF </a> o usa el botón "Descargar".</p>
</object>
</details>

!!! info "Aviso"
    Documento con marca de agua para distribución **digital**.

## Cómo citar
> Jean-Paul Brasselet, Felipe Cano, Dominique Cerveau, Dung, Tráng Lê, Frank Loray, Mutsuo Oka, José Seade, Mark Spivakovsky. (2999.0). *Teoría de singularidades en topología, geometría y foliaciones II*. Instituto de Matemáticas, UNAM, 0.0

<details>
  <summary>BibTeX</summary>
  <p style="font-family:'Courier New'">@BOOK{pap-not-2, <br>title = {Teoría de singularidades en topología, geometría y foliaciones II}, <br>author = {Brasselet, Jean-Paul and Cano, Felipe and Cerveau, Dominique and Lê, Dung and Loray, Frank and Oka, Mutsuo and Seade, José and Spivakovsky, Mark}, <br>year = {2999.0}, <br>publisher = {Instituto de Matemáticas, UNAM}, <br>address = {México}} </p>
</details>


[Volver al catálogo](../catalogo.md)

[Explorar](../explorar.md)
